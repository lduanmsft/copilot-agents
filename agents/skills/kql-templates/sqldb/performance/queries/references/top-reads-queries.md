...
This skill analyzes query performance by identifying the top queries with the highest logical read counts. Logical reads indicate the number of pages read from the buffer cache, which is a key metric for identifying IO-intensive queries that may be impacting database performance.
...
| summarize TotalLogical_reads = sum(logical_reads),totalPhysical_reads=sum(physical_reads),totalPhysical_io_reads=sum(physical_io_reads)  by query_hash
| order by TotalLogical_reads desc nulls first
| take {TopN}
```
